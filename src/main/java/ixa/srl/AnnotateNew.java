...
public class AnnotateNew {
	...
	public AnnotateNew(PreProcessNew preprocess) {
		mate = new ParseNew(preprocess);
	}
	...
	public void SRLToKAF(KAFDocument kaf, String lang, String option)
		...
		if (lang.equals("eng")) {
			...
			this.kaflang = "es";
                ...
                KAFDocument.LinguisticProcessor depsLP = kaf.addLinguisticProcessor("deps", "ixa-pipe-srl-" + kaflang, "1.0");
                depsLP.setBeginTimestamp();
		KAFDocument.LinguisticProcessor srlLP = kaf.addLinguisticProcessor("srl", "ixa-pipe-srl-" + kaflang, "1.0");
...
//		}
	...
	private List<String> KAF2Mate(HashMap<String, String> annotationlines,
		...
		for (WF wf : wfs) {
			...
			if (kaf.getBySent(KAFDocument.AnnotationType.TERM, sentence).size() < 100) { //Limit the size of the sentence
...
if (kaf.getBySent(KAFDocument.AnnotationType.TERM, sentence).size() < 100) { //Limit the size of the sentence
	...
	private void XMLMate2KAFSRL(KAFDocument kaf, Document doc) {
		...
		PredicateMatrixNew PM = new PredicateMatrixNew();
